...
# sedCode(sedCmd)
function sedCode() {
  find . -type f -not \( -path "./bootstrap.sh" -o -path "*/.git/*" -o -path "*/build/*" -o -path "*/.gradle/*" -o -path "*/.creek/*" \) -print0 | xargs -0 sed -i "$1"
}
...
find . -type f -path "*$oldBaseDir*" -not \( -path "./bootstrap.sh" -o -path "*/.git/*" -o -path "*/build/*" -o -path "*/.gradle/*" -o -path "*/.creek/*" \) -exec bash -c '
    newPath=${3/$1/$0}
